...
function Meta(m)
    ...
    -- (reader-logo / reader-logo-en) hat Vorrang.
    ...
    if quarto.doc.is_format("typst") then
        ...
        if not m['reader-logo'] then
            m['reader-logo'] = pandoc.MetaString(logo_rel("thws-logo.svg"))
        end
        if not m['reader-logo-en'] then
            m['reader-logo-en'] = pandoc.MetaString(logo_rel("thws-logo_en.svg"))
        end
    end
